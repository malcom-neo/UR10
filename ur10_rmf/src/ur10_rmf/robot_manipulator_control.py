...
MOTION_CONFIG_PATH = "motion_config.yaml"
ROS_TIME_RATE = 6 #hz
...
class RobotManipulatorControl():
  ...
  def load_motion_config(self, path):
    ...
    #dir_path = dir_path.strip("scripts") + "config/"
    dir_path = dir_path.strip("src/ur10_rmf")
    
    full_path="/"+dir_path +"s/src/ur10_rmf/config/" +path
    #full_path = dir_path.strip("scripts") + path
    ...
    if (self.log_motion_error == True):
      self.log_file = open(full_path+"error_log.txt", "w+")
  ...
  def default_config(self):
    # default_position = self.yaml_obj['motion']['M3']['data']
    # timeFactor = self.yaml_obj['motion']['M3']['timeFactor']
    # self.ur10.go_to_joint_state(default_position,timeFactor)
    self.execute_motion("M3")
  ...
  def open_gripper(self): 
    ...
    if (self.enable_gripper == True):
      ...
      while (self.gripper_state == 1):
        if (self.gripper_state == -1):   #nani dafuk
  ...
  def get_coeff_from_id(self, ch, id):
    char_idx = id.find(ch)
    filtered_id = id[char_idx:len(id)] #remove m so just left num
