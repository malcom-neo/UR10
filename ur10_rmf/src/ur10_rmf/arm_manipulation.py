...
# Change Quaternion pose with input delta rpy, arg can be 0 or quaternion msg
...
class ArmManipulation(object):
  ...
  def addModelScene(self):
    print (colored("Adding space constraint!",'white','on_green'))
    ...
    box.header.frame_id = "base_link"
    box.pose.orientation.w = 1.0
    box.pose.position.x = 1.2
    #box.pose.position.y = 0.5
    box.pose.position.z = 0.6
    self.scene.add_box("platform", box, size=(0.61, 1.21, 0.032))
  ...
  def go_to_joint_state(self, joint_goal, time_factor):
    ...
    state = self.robot.get_current_state()  # robotcommander
    self.group.set_start_state(state)   #movegroupcommander class
    group = self.group
  ...
  def go_to_pose_goal(self, pose_list, time_factor ):
    ...
    #self.set_constrains(pose_goal)
    #group.set_pose_target(pose_goal)

    # Planner to compute the plan and execute it.
    plan = group.plan (joints = pose_goal) #movegroup commander
